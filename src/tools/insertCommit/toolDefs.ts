...
export const INSERT_COMMIT_TOOL: Tool<{
  ...
  `,
  schema: {
    prIndex: z.number().int().min(0, "PR index must be a non-negative integer")
      .describe("Index of the PR to insert the commit into. Zero-based index in the PR array."),
    insertAfterCommitIndex: z.number().int()
      .describe("Insert after this commit index. Use -1 to insert at the beginning."),
    goal: z.string().min(1, "Goal must be a non-empty string").max(60, goalLengthErrorMessage)
      .describe("Description of what this commit aims to accomplish. Should match the format of an actual commit message - concise and specific."),
  ...
  handler: async (params, extra: RequestHandlerExtra) => {
    try {
      logger.info(`InsertCommit tool called for PR #${params.prIndex} after commit #${params.insertAfterCommitIndex}`);
      ...
      const result = workPlan.insertCommit(insertParams, String(params.agentId), String(params.workplanId));
