...
export const UPDATE_STATUS_TOOL: Tool<{
  ...
  schema: {
    prIndex: z
      ...
      .describe(
        'Index of the PR containing the commit to update. Zero-based index in the PR array.',
      ),
    commitIndex: z
      .number()
      .int()
      .describe(
        'Index of the commit to update within the specified PR. Zero-based index in the commit array. Use -1 to update the PR directly.',
      ),
    status: z
      .enum([
        'not_started',
      ...
      .describe(
        "New status value for the commit. Valid values: 'not_started' (work has not begun), 'in_progress' (currently being worked on), 'user_review' (waiting for user to review), 'completed' (work is finished), 'cancelled' (work has been abandoned), or 'needsRefinment' (needs further refinement).",
      ),
    goal: z
      .string()
      ...
      .optional()
      .describe(
        'New goal description for the commit. Optional - only provide if you want to change the commit goal.',
      ),
    developerNote: z
      .string()
      .optional()
      .describe(
        'Developer implementation notes. Can be added to both PRs and commits to document important implementation details.',
      ),
    agentId: z
      .string()
    ...
    workplanId: z
      ...
      .describe(
        'Required identifier for which workplan to update. Reuse the same workplanId for the current ticket/thread; do not create a new one per prompt. Only switch when the user explicitly requests it.',
      ),
  },
  handler: async (params, extra: RequestHandlerExtra) => {
    try {
      ...
      const result = workPlan.updateStatus(
        updateParams,
