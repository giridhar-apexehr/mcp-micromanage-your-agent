...
export const TRACK_TOOL: Tool<{
  ...
  schema: {
    ...
    workplanId: z
      ...
      .describe(
        'Required identifier for which workplan to track. Reuse the same workplanId for the current ticket/thread; do not create a new one per prompt. Only switch when the user explicitly requests it.',
      ),
    prIndex: z
      .number()
      .int()
      ...
      .optional()
      .describe(
        'Optional: Index of the PR to include in the response. Zero-based index in the PR array.',
      ),
  },
