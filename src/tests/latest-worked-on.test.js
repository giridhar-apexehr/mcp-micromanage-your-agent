...
describe('latestWorkedOn tracking', () => {
  test('updates latestWorkedOn only on commit status transitions into in_progress/user_review/completed and mirrors to agents.json', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mcp-micromanage-latest-worked-'));
    ...
    try {
      ...
      expect(agentsIndex.agents[agentId].workplans[workplanId].latestWorkedOn).toBeUndefined();
      ...
      expect(agentsIndex.agents[agentId].workplans[workplanId].latestWorkedOn).toBeDefined();
      expect(agentsIndex.agents[agentId].workplans[workplanId].latestWorkedOn.status).toBe('in_progress');
      ...
      const resNote = wp.updateStatus(
        { prIndex: 0, commitIndex: 0, status: 'in_progress', developerNote: 'note' },
        agentId,
        workplanId,
      );
      ...
      expect(agentsIndex.agents[agentId].workplans[workplanId].latestWorkedOn.at).toBe(atAfterInProgress);
      ...
      expect(agentsIndex.agents[agentId].workplans[workplanId].latestWorkedOn.status).toBe('user_review');
      ...
      expect(agentsIndex.agents[agentId].workplans[workplanId].latestWorkedOn.status).toBe('completed');
    } finally {
