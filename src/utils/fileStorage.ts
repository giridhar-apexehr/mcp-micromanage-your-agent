...
export function acquireFileLock(
  ...
  while (true) {
    try {
      ...
      if (attempt >= retries) {
        throw new Error(
          `Failed to acquire file lock after ${retries} retries: ${filePath}`,
        )
      }
      sleepSync(delay)
...
export function writeJsonAtomic(filePath: string, data: unknown): boolean {
  try {
    ...
    logger.logError(
      `Failed to atomically write JSON to file: ${filePath}`,
      error,
    )
    try {
      const dir = path.dirname(filePath)
...
export function fileExists(): boolean {
  ...
  logger.debug(
    `Checking if file exists: ${filePath} - ${exists ? 'Yes' : 'No'}`,
  )
  return exists
}
...
ensureDirectoryExists(dataDir)
logger.info(
  `File storage initialized with data directory: ${dataDir} and file name: ${dataFileName}`,
)
