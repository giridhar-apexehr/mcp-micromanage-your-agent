 ...
 */
export const generatePRSummaries = (
...
export const updatePRStatusBasedOnCommits = (pr: PullRequest): PullRequest => {
  ...
  const nonCancelledCommits = commits.filter(
    (commit) => commit.status !== 'cancelled',
  )
  ...
  if (
    nonCancelledCommits.length === 0 &&
    commits.some((commit) => commit.status === 'cancelled')
  ) {
    updatedPr.status = 'cancelled'
  } else if (
    nonCancelledCommits.length > 0 &&
    nonCancelledCommits.every((commit) => commit.status === 'completed')
  ) {
    ...
    nonCancelledCommits.some((commit) => commit.status === 'user_review')
  ) {
    updatedPr.status = 'user_review'
  } else if (
    nonCancelledCommits.some((commit) => commit.status === 'needsRefinment')
  ) {
    updatedPr.status = 'needsRefinment'
  } else if (
    nonCancelledCommits.some((commit) => commit.status === 'completed')
  ) {
    updatedPr.status = 'in_progress'
  } else {
 ...
 */
export const updatePRDeveloperNote = (
  pr: PullRequest,
  note: string,
): PullRequest => {
  return {
 ...
 */
export const updateCommitDeveloperNote = (
  pr: PullRequest,
  commitIndex: number,
  note: string,
): PullRequest => {
