...
export class WorkPlan {
  ...
  private loadTicketFromFile(
    ...
    try {
      ...
      if (
        typeof ticket.goal !== 'string' ||
        !Array.isArray((ticket as { pullRequests?: unknown }).pullRequests)
      ) {
        return 'noTicket'
      }
  ...
  private migrateWorkplanFilesFromLegacyIfNeeded(): void {
    ...
    for (const [agentId, agentState] of Object.entries(this.agents)) {
      for (const [workplanId, ticket] of Object.entries(
        agentState.workplans ?? {},
      )) {
        if (ticket === 'noTicket') {
          continue
        }
    ...
    logger.info(
      `Migrated ${writtenCount} workplan file(s) (${skippedCount} skipped)`,
    )
  }
    ...
    const updated = fileStorage.updateAgentsIndex((state) => {
      ...
      for (const [agentId, agentState] of Object.entries(this.agents)) {
        ...
        for (const [workplanId, ticket] of Object.entries(
          ...
          const commitCount = ticket.pullRequests.reduce(
          ...
          next.agents[agentId].workplans[workplanId] = {
            ...
            ...(ticket.latestWorkedOn
              ? { latestWorkedOn: ticket.latestWorkedOn }
              : {}),
          }
        }
      }
    ...
    try {
      if (!this.initialized) {
        return errorResponse(
          'WorkPlan is not initialized. Call initialize() first.',
        )
      }

      logger.info(
        `Inserting commit for PR #${input.prIndex} after commit #${input.insertAfterCommitIndex}`,
      )
      ...
      const workingTicket =
        fileTicket !== 'noTicket'
          ? fileTicket
          : (agentState.workplans[workplanId] ?? 'noTicket')
      ...
      const inProgressCommits = ticket.pullRequests.reduce(
        (sum: number, pullRequest: PullRequest) => {
          return (
            sum +
            pullRequest.commits.filter(
              (c: { status: Status }) => c.status === 'in_progress',
            ).length
          )
        },
        0,
      ...
      const perWorkplanSaved = this.saveTicketToFile(
        agentId,
        workplanId,
        ticket,
      )
      if (!perWorkplanSaved) {
        logger.warn(
          `Failed to persist per-workplan ticket file for agentId=${agentId}, workplanId=${workplanId}`,
        )
      } else {
        const prCount = ticket.pullRequests.length
        const commitCount = ticket.pullRequests.reduce(
        ...
        if (!indexUpdated) {
          logger.warn(
            `Failed to update agents index for agentId=${agentId}, workplanId=${workplanId}`,
          )
        }
      }
  ...
  public initialize(options: WorkPlanInitOptions = {}): boolean {
    try {
      ...
      logger.info(
        `WorkPlan initialized successfully. Using data file: ${fileStorage.getDataFilePath()}`,
      )
      return true
    } catch (error) {
  ...
  private loadState(): void {
    try {
      ...
      if (agentsIndexExists) {
        ...
        logger.info(
          `WorkPlan state loaded from agents index: ${fileStorage.getAgentsIndexPath()}`,
        )
        return
      }
      ...
      const hasAgentsSchema =
        typeof savedState.agents !== 'undefined' && savedState.agents !== null
      const hasWorkplansSchema =
        typeof savedState.workplans !== 'undefined' &&
        savedState.workplans !== null
      const hasLegacySchema =
        typeof savedState.currentTicket !== 'undefined' &&
        !hasAgentsSchema &&
        !hasWorkplansSchema

      if (hasAgentsSchema) {
        const rawAgents =
          savedState.agents && typeof savedState.agents === 'object'
        ...
        this.agents = Object.fromEntries(
          Object.entries(rawAgents).map(([agentId, agentState]) => {
            ...
            return [
              agentId,
              {
                workplans: maybeState.workplans ?? {},
              } satisfies AgentWorkPlanState,
            ]
          }),
        )
        ...
        if (savedState.version && savedState.version !== this.version) {
          logger.warn(
            `Data version mismatch: file=${savedState.version}, current=${this.version}`,
          )
        }
        ...
        const migratedAgentState: AgentWorkPlanState = {
          workplans:
            savedState.workplans && typeof savedState.workplans === 'object'
        ...
        logger.info(
          'Pre-agent workplan schema detected; migrated to agent-scoped format',
        )
        this.saveState()
        ...
        const migratedState: WorkPlanState = {
          agents: {
            [migratedAgentId]: {
              workplans: {
                [migratedWorkplanId]: savedState.currentTicket ?? 'noTicket',
              },
            },
          },
          lastUpdated: savedState.lastUpdated || this.lastUpdated,
        ...
        logger.info(
          'Legacy single-ticket schema detected; migrated to agent-scoped format',
        )
        this.saveState()
      } else {
      ...
      logger.info(
        `WorkPlan state loaded from file: ${fileStorage.getDataFilePath()}`,
      )
  ...
  private saveState(): boolean {
    try {
      ...
      if (success) {
        if (this.legacyWriterEnabled) {
          logger.info(
            `WorkPlan state saved to file: ${fileStorage.getDataFilePath()}`,
          )
        } else {
  ...
  public plan(
    ...
    try {
      ...
      if (!agentId || !String(agentId).trim()) {
        return errorResponse(
          'agentId is required. Provide a unique identifier for the calling agent.',
        )
      }

      if (!workplanId || !String(workplanId).trim()) {
        return errorResponse(
          'workplanId is required. Provide a unique identifier for this workplan.',
        )
      }

      logger.info(
        `Creating plan with goal: ${input.goal}, ${input.prPlans.length} PRs`,
      )
      ...
      if (!perWorkplanSaved) {
        ...
        const commitCount = newTicket.pullRequests.reduce(
          (sum: number, pr: PullRequest) => sum + pr.commits.length,
          0,
        )
      ...
      const agentState: AgentWorkPlanState = this.agents[agentId] ?? {
        workplans: {},
      }
            ...
            text: JSON.stringify(
              {
                ...
                commitCount: newTicket.pullRequests.reduce(
                  (sum: number, pr: PullRequest) => sum + pr.commits.length,
                  0,
                ),
                message,
  ...
  public trackProgress(
    ...
    try {
      ...
      const ticketCheck = ensureTicketExists(
        agentState.workplans[workplanId] ?? 'noTicket',
      )
      if (!ticketCheck.result) {
        logger.warn('No implementation plan found')
      ...
      if (prIndex !== undefined) {
        const isValidPrIndex =
          Number.isInteger(prIndex) &&
          prIndex >= 0 &&
          prIndex < ticket.pullRequests.length
        if (!isValidPrIndex) {
          const availablePrIndices = ticket.pullRequests.map(
            (_, index) => index,
          )
          return {
            content: [
  ...
  public updateStatus(
    ...
    try {
      ...
      logger.info(
        `Updating status for PR #${input.prIndex}, commit #${input.commitIndex} to "${input.status}"`,
      )
      ...
      if (!validationResult.isValid) {
        const errorMessage =
          validationResult.errorMessage || 'ステータス遷移が無効です'
      ...
      if (input.status === 'in_progress') {
        ...
        ticket.pullRequests.forEach((pullRequest, pullRequestIndex) => {
          pullRequest.commits.forEach((commit, commitIdx) => {
            ...
            if (commit.status === 'in_progress') {
              ...
              resetTasksCount++
              logger.info(
                `Reset task PR #${pullRequestIndex}, commit #${commitIdx} from "in_progress" to "not_started"`,
              )
            }
          })
        })
        ...
        if (resetTasksCount > 0) {
          changes.push(
            `${resetTasksCount} other in-progress tasks reset to "not_started"`,
          )
        }
      }
