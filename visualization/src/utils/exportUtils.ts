...
export const exportToMarkdown = (workplan: WorkPlan): string => {
  ...
  const completedPRs = workplan.prPlans.filter(
    (pr) =>
      pr.status === 'completed' ||
      pr.commitPlans.every((commit) => commit.status === 'completed'),
  ).length

  const totalCommits = workplan.prPlans.reduce(
    (total, pr) => total + pr.commitPlans.length,
    0,
  )
  const completedCommits = workplan.prPlans.reduce(
    (total, pr) =>
      total +
      pr.commitPlans.filter((commit) => commit.status === 'completed').length,
    0,
  )
  ...
  workplan.prPlans.forEach((pr, prIndex) => {
    const prStatus =
      ...
      (pr.commitPlans.every((commit) => commit.status === 'completed')
        ? 'completed'
        : pr.commitPlans.some((commit) => commit.status === 'in_progress')
          ? 'in_progress'
          : 'not_started')
...
export const exportToCSV = (workplan: WorkPlan): string => {
  let csv =
    'PR Number,PR Goal,PR Status,Commit Number,Commit Goal,Commit Status\n'
