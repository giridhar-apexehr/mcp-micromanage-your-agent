...
const validateWorkPlan = (data: unknown): WorkPlan => {
  ...
  const validatedPrPlans: PRPlan[] = data.prPlans.map((pr: unknown): PRPlan => {
    if (
      !isRecord(pr) ||
      typeof pr.goal !== 'string' ||
      !Array.isArray(pr.commitPlans)
    ) {
    ...
    const status =
      ...
      VALID_STATUSES.includes(statusValue as CommitStatus)
        ? (statusValue as CommitStatus)
        : undefined

    // Validate and normalize commit plans
    const commitPlans: CommitPlan[] = pr.commitPlans.map(
      (commit: unknown): CommitPlan => {
        ...
        const commitStatus =
          typeof commitStatusValue === 'string' &&
          VALID_STATUSES.includes(commitStatusValue as CommitStatus)
