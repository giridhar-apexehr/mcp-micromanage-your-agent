 ...
 */
export const usePolling = ({ loadData }: UsePollingParams): UsePollingResult => {
  ...
  const currentPollingSeconds = useMemo(() => Math.max(1, Math.round(pollingIntervalMs / 1000)), [pollingIntervalMs]);
  const parsedDraftSeconds = useMemo(() => Number(draftPollingSeconds), [draftPollingSeconds]);
  const draftSecondsValid = useMemo(() => Number.isFinite(parsedDraftSeconds) && parsedDraftSeconds > 0, [parsedDraftSeconds]);
  const draftSecondsForSelection = useMemo(() => (draftSecondsValid ? Math.round(parsedDraftSeconds) : currentPollingSeconds), [draftSecondsValid, parsedDraftSeconds, currentPollingSeconds]);
